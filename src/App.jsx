...
const inlineStyles = `
  ...
  /* --- MICRO-AJUSTE: ESTILO DE LA TARJETA --- */
  .frecuencia-card {
    transition: all 0.3s ease;
    padding: 20px 10px; /* Un poco más de aire */
    border-radius: 40px;
    border: 1px solid rgba(34, 211, 238, 0.15); /* Más sutil */
    ...
    align-items: center;
    min-height: 100px; /* Alto mínimo para coherencia */
    height: auto; /* Permite que crezca con descripciones largas */
  }

  /* --- MEDIA QUERY: PRECISIÓN QUIRÚRGICA PARA MÓVIL (Paso 2.1.1) --- */
  @media (max-width: 768px) {
    ...
    .frecuencia-name {
      font-size: 11px !important; /* Reducción sutil del tamaño de fuente */
      letter-spacing: 1px !important; /* Espaciado más fino */
    }
    .frecuencia-hz {
...
const App = () => {
  ...
  // Lista de frecuencias (Próximamente categorizada)
  const tracks = [
  ...
  // --- BLOQUE 1: SPLASH (BLINDADO - INTOCABLE) ---
  if (showSplash) {
    return (
  ...
  // --- BLOQUE 2: REPRODUCTOR (BLINDADO - INTOCABLE) ---
  if (selectedTrack) {
    return (
  ...
  // --- BLOQUE 3: LISTA (EDICIÓN QUIRÚRGICA CON GUANTES DE SEDA PASO 2.1.1) ---
  return (
    <div className="fade-in-smooth" style={{ backgroundColor: '#020617', minHeight: '100vh', color: 'white', padding: '15px', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        ...
        {/* Tabs de navegación (frecuencias/meditaciones) */}
        ...
        {/* GRILLA DE FRECUENCIAS (Con micro-ajustes para móvil) */}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px', width: '100%', maxWidth: '480px', paddingBottom: '30px' }}>
          {tracks.filter(t => t.type === activeTab && t.name.toLowerCase().includes(searchTerm.toLowerCase())).map(track => (
            <div key={track.id} onClick={() => setSelectedTrack(track)} className="frecuencia-card">
              {/* MICRO-AJUSTE: Tipografía de la tarjeta con clases específicas para el blindaje */}
              <div className="frecuencia-name" style={{ fontSize: '13px', fontWeight: '300', color: 'white', textTransform: 'uppercase', letterSpacing: '2px' }}>{track.name}</div>
              <div className="frecuencia-hz" style={{ fontSize: '9px', color: accentColor, marginTop: '8px', fontWeight: 'bold', letterSpacing: '1px' }}>{track.hz}</div>
            </div>
          ))}
        </div>
      </div>
