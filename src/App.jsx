...
const App = () => {
  ...
  const tracks = [
    ...
    { id: "03", name: "Alpha Void", hz: "8 – 13 Hz", type: "frecuencias", desc: "Punto cero de la consciencia. Silencio total para el reordenamiento genético." },
    { id: "04", name: "Alpha Origen", hz: "8 Hz", type: "frecuencias", desc: "Conexión directa con la frecuencia Schumann y la resonancia primordial." },
    { id: "05", name: "Gaia Vision", hz: "8,3 Hz", type: "frecuencias", desc: "Expansión de la percepción sensorial y conexión planetaria." },
    { id: "06", name: "Alpha Voice", hz: "8,22 Hz", type: "frecuencias", desc: "Sintoniza la expresión de tu verdad interna con tu campo vibratorio." },
    { id: "M1", name: "Coherencia del Ser", hz: "963 Hz", type: "meditaciones", desc: "Sincroniza corazón y mente en una paz inquebrantable." }
  ];
  ...
  // --- REPRODUCTOR COMPACTO CON TÍTULO EXTRA ---
  if (selectedTrack) {
    return (
      <div className="fade-in" style={{ backgroundColor: '#020617', minHeight: '100vh', color: 'white', padding: '15px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
        <style>{inlineStyles}</style>
        <button onClick={() => { setSelectedTrack(null); setIsPlaying(false); }} style={{ position: 'absolute', top: '25px', left: '20px', background: 'none', border: 'none', color: 'white', fontSize: '24px', opacity: 0.4 }}>✕</button>
        
        {/* TÍTULO ÁLBUM (Novedad) */}
        <p style={{ fontSize: '9px', letterSpacing: '3px', color: accentColor, opacity: 0.9, marginBottom: '20px', textTransform: 'uppercase' }}>
          Resonancia Origen • Álbum Alpha 1
        </p>

        <div style={{ position: 'relative', width: '160px', height: '160px', marginBottom: '20px', borderRadius: '50%', border: `4px solid #001a33`, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: `0 0 40px ${accentColor}33`, animation: isPlaying ? 'breathe 4s infinite ease-in-out' : 'none' }}>
        ...
        <h2 style={{ fontSize: '20px', fontWeight: '200', letterSpacing: '4px', textTransform: 'uppercase', marginBottom: '5px' }}>{selectedTrack.name}</h2>
        <p style={{ color: accentColor, fontSize: '10px', letterSpacing: '3px', fontWeight: 'bold', marginBottom: '10px' }}>{selectedTrack.hz}</p>
        <p style={{ fontSize: '11px', color: '#fdfcf5', opacity: 0.7, maxWidth: '280px', lineHeight: '1.4', marginBottom: '25px' }}>"{selectedTrack.desc}"</p>
        
        <div style={{ display: 'flex', gap: '10px', marginBottom: '30px' }}>
          {[15, 30, 60, '∞'].map((time) => (
            <button key={time} onClick={() => setSelectedTime(time)} className="time-button" style={{ width: '50px', padding: '7px 0', border: `1px solid ${selectedTime === time ? accentColor : 'rgba(255,255,255,0.1)'}`, background: selectedTime === time ? `${accentColor}22` : 'none', color: 'white', fontSize: '10px' }}>{time === '∞' ? time : `${time}'`}</button>
          ))}
        </div>
        <button onClick={() => setIsPlaying(!isPlaying)} style={{ width: '70px', height: '70px', borderRadius: '50%', border: `1px solid ${accentColor}`, background: isPlaying ? `${accentColor}11` : 'none', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
  ...
  // --- LISTA PRINCIPAL CON BUSCADOR FIJO ---
  return (
    <div className={isReady ? "fade-in" : ""} style={{ backgroundColor: '#020617', minHeight: '100vh', color: 'white', padding: '15px', fontFamily: 'sans-serif', opacity: isReady ? 1 : 0 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%' }}>
        <div style={{ position: 'relative', width: '160px', height: '160px', marginBottom: '55px', borderRadius: '50%', border: `4px solid #001a33`, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: `0 0 50px ${accentColor}44`, animation: 'breathe 4s infinite ease-in-out' }}>
        ...
        {/* BUSCADOR CON CENTRADO ABSOLUTO */}
